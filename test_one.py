...
class GameOperationMethod(object):
    ...
    def _Game_table_check(self):
        ...
        try:
            if self.pocos("btn_call").wait(1).exists():  #小盲位平跟
                self.pocos("btn_call").wait(2).click()
            else:
    ...
    def _Game_table_raise(self):
        """加注"""
        try:
            self.pocos("btn_add").wait(19).click()
    ...
    def _Game_table_fold(self):
        """弃牌"""
        self.pocos("btn_fold").wait(19).click()
    ...
    def _Game_table_sitdown(self):
        ...
        try:
            ...
            if pocos("panel_expand").child("btn_find_bb").wait(2).exists():
                print("确认成功并成功弹出Wait for BB")                
            
            elif pocos("panel_warn").child("lbl_tip_warn").wait().exists():#余额不足判断
    ...
    def test_dddd():
        """保险与多次牌处理"""
        if pocos("panel_content").wait(2).exists(): #保险与多次牌处理
            print("关闭弹窗")
            pocos("btn_close")       #叉处理按钮
