...
def Mtt_Create(
    ...
    if Re_Create.status_code == 200:
        if msg["error_msg"] == '':#error_msg为空时，赛事已满
            ...
            #获取2个赛事id
            register_MTT(event_uuid, event_id)
    else:
        print('消息发送失败，请根据信息检查参数')
    #判断信息是否发送成功，或者服务器MTT进程已满

def register_MTT(event_uuid,event_id):
...
if __name__=='__main__':
    ...
    # Crm_login(get_sign())    #执行登陆验证   #获取csrfmiddlewaretoken
        raise Exception('别整这些没用的')
    else:
        url = f'http://192.168.101.{host}:8109'
    # 第二部给出创桌的url
    mode = input("请选择创建方式，输入0为默认德州，输入1为默认PLO，输入2为mpc，输入3为自定义：")
    # 选择模式
    if mode == "0":
        game_type = 'Holdem'
        name=game_type
        ...
        name='MPC'
        is_mpc='1'
        mtt_type=input('请按照以下规则输入Value值\n'
                       ' value="1" > Day1\n'
                       ' value="2" > Day2\n'
                       ' value="2" > Other\n'
                       ':')
        series_day=input('请按照以下规则输入Value值\n'
                         ' value="1" > May 22\n'
                         ' value="2" > May 23\n'
                         ' value="3" > May 24\n'
                         ' value="4" > May 25\n'
